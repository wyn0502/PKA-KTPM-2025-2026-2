...
const sbApi = {
    ...
    async getCheatingLogsByUser(examId, userId) {
        const { data } = await supabase.from('cheating_logs').select('*').eq('exam_id', examId).eq('user_id', userId).order('timestamp', { ascending: true });
        return data || [];
    },
...
export const api = {
    ...
    getCheatingLogsByUser:  async (eid, uid)  => isDemo ? []                                 : sbApi.getCheatingLogsByUser(eid, uid),
