...
const sbApi = {
    ...
    async addExam({ title, subject_id, duration_minutes, shuffle_questions, shuffle_answers, show_result, status, start_time, end_time, question_ids }) {
    ...
    async updateExam(id, { title, subject_id, duration_minutes, shuffle_questions, shuffle_answers, show_result, status, start_time, end_time, question_ids }) {
    ...
    async addStudent(d) {
        const { data, error } = await supabase.from('users').insert({ ...d, role: 'student', password_hash: '' }).select().single();
    ...
    async getCheatingLogs(examId) {
        const q = supabase.from('cheating_logs').select('*, users(full_name)').order('timestamp', { ascending: false });
        const { data } = examId ? await q.eq('exam_id', examId) : await q;
        return (data || []).map(l => ({ ...l, user_name: l.users?.full_name }));
    },
    ...
    async getActiveSessions(examId) {
        const { data } = await supabase.from('active_sessions').select('*, users(full_name, student_id)').eq('exam_id', examId);
        return (data || []).map(s => ({ ...s, user_name: s.users?.full_name, student_id: s.users?.student_id }));
    },
    async registerSession({ exam_id, user_id, total_questions }) {
        await supabase.from('active_sessions').upsert({ exam_id, user_id, total_questions, status: 'in_progress', last_activity: new Date().toISOString() });
        return { success: true };
    },
