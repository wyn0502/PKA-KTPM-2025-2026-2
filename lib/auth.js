...
export function AuthProvider({ children }) {
    ...
    useEffect(() => {
        ...
        // Supabase mode
        // Cache last-known profile in localStorage so F5 doesn't cause logout or wrong role
        const cacheProfile = (p) => { try { localStorage.setItem('_qp', JSON.stringify(p)); } catch {} };
        const getCached   = ()  => { try { return JSON.parse(localStorage.getItem('_qp')); } catch { return null; } };
        const clearCache  = ()  => { try { localStorage.removeItem('_qp'); } catch {} };
        ...
        supabase.auth.getSession().then(async ({ data: { session } }) => {
            if (session?.user) {
                // Show cached profile immediately — always stop spinner now
                const cached = getCached();
                if (cached) setUser(cached);
                settle(); // stop spinner regardless of cache

                // Refresh profile in background (non-blocking)
                fetchProfile(session.user.email).then(profile => {
                    if (profile) { setUser(profile); cacheProfile(profile); }
        ...
        }).catch(() => { setUser(null); settle(); });

        // Handle login/logout/token-refresh after initial load
        const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
            if (event === 'INITIAL_SESSION') return; // handled by getSession above
            if (event === 'SIGNED_OUT') { clearCache(); setUser(null); settle(); return; }
            if (session?.user) {
        ...
        try {
            ...
            if (!input.includes('@')) {
                const { data: found } = await supabase
                    ...
                    .select('email')
                    .eq('student_id', input)
                    .maybeSingle(); // maybeSingle returns null (no error) when 0 rows found
            ...
            try { localStorage.setItem('_qp', JSON.stringify(profile)); } catch {}
            setUser(profile);
    ...
    const logout = useCallback(async () => {
        ...
        try { localStorage.removeItem('_qp'); } catch {}
