...
export function AuthProvider({ children }) {
    ...
    useEffect(() => {
        ...
        supabase.auth.getSession().then(async ({ data: { session } }) => {
            if (session?.user) {
                // Show cached profile immediately so spinner stops and role is correct
                const cached = getCached();
                if (cached) { setUser(cached); settle(); }

                // Fetch fresh profile in background
                const profile = await fetchProfile(session.user.email).catch(() => null);
                if (profile) { setUser(profile); cacheProfile(profile); }
                else if (!cached) { setUser(null); } // no cache & fetch failed → force login
                settle();
            } else {
                clearCache();
                setUser(null);
